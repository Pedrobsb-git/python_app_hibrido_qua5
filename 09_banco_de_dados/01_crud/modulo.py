...
def cadastrar(session, Pessoa):
    try:
        ...
        genero = input("Informe seu Gênero: ").strip()
                ...
                limpar()
                  match campo:
                        ...
                        case "5":
                            ...
                            novo_nascimento = datetime.strptime(novo_nascimento,"%d/%m/%Y").date() if novo_nascimeto != "" else pessoa.nascimento
...
limpar()
    match opcao:
        case "1":
            ...
            pessoa = session.query(Pessoa).filter_by(id_pessoa=id_pessoa).first()
        case "2":
            email = input("Iforme o E-mail do cadastro a ser excluido: ").strip().lower()
            pessoa = session.query(Pessoa).filter_by(email=email).first()
        case "3"
            return ""
        case _:
...
if pessoa:
    ...
    print("1 - Sim")
    print("2 - Não")
    excluir = intput("Tem certeza de que deseja excluir o registro? ").strip()
    match excluir:
        case "1":
